...
template <typename T>
void checkDeviceFloats(T* a, int size, int stride, int line_stride, int count, cudaStream_t stream)
{
    T* a_h = (T*) malloc(size);
    cudaMemcpyAsync(a_h, a, size, cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    for (int i = 0; i < count; i++)
    {
        printf("%f ", (float) a_h[i * stride]);
        if (i % line_stride == line_stride - 1)
            printf("\n");
    }
    printf("\n");
    free(a_h);
