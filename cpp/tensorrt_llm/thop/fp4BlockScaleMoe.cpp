...
std::vector<torch::Tensor> run_fp4_block_scale_moe_runner(torch::optional<torch::Tensor> const& routing_logits,
    ...
    torch::optional<torch::Tensor> const& out_tensor = torch::nullopt, int64_t const fc2_scale_rule = 0,
    double const fc2_input_scale = 0.0, double const adaptive_quant_range = 1536.0)
{
    TORCH_CHECK(dtype == btg::Dtype::E4m3 || dtype == btg::Dtype::E2m1, "dtype can only be e4m3 or e2m1.");
    TORCH_CHECK(fc2_scale_rule >= 0 && fc2_scale_rule <= 3,
        "fc2_scale_rule must be 0 (disabled), 1 (MSE), 2 (MAE), or 3 (ABS_MAX).");
    if (fc2_scale_rule != 0)
    {
        TORCH_CHECK(dtype == btg::Dtype::E2m1, "Adaptive FC2 quantization requires E2M1 activations.");
        TORCH_CHECK(std::isfinite(fc2_input_scale) && fc2_input_scale > 0.0,
            "fc2_input_scale must be finite and greater than zero when adaptive FC2 quantization is enabled.");
        TORCH_CHECK(std::isfinite(adaptive_quant_range) && adaptive_quant_range > 0.0,
            "adaptive_quant_range must be finite and greater than zero when adaptive FC2 quantization is enabled.");
    }
    ...
    if (fc2_scale_rule != 0)
    {
        // The adaptive amax reduction scans every allocated FC1 row. Clearing
        // padding prevents unwritten entries from affecting the dynamic scale.
        gemm1_output.zero_();
    ...
    at::Tensor adaptive_bf16_buf;
    at::Tensor adaptive_amax_buf;
    at::Tensor adaptive_amax_output;
    at::Tensor adaptive_retirement_count;
    at::Tensor adaptive_fc2_input_scale;
    at::Tensor corrected_fc2_alpha;
    if (fc2_scale_rule != 0)
    {
        int64_t const max_padded_tokens = max_num_padded_tokens_gemm1;
        int const sm_count = tensorrt_llm::common::getMultiProcessorCount();

        adaptive_bf16_buf = at::detail::empty_cuda(
            {max_padded_tokens, intermediate_size}, at::ScalarType::BFloat16, routing_device, std::nullopt);
        adaptive_amax_buf = at::detail::empty_cuda(
            {sm_count * 4}, at::ScalarType::Float, routing_device, std::nullopt);
        adaptive_amax_output = at::detail::empty_cuda({2}, at::ScalarType::Float, routing_device, std::nullopt);
        adaptive_retirement_count = at::zeros(
            {1}, at::TensorOptions().device(routing_device).dtype(at::ScalarType::Int));
        adaptive_fc2_input_scale = at::full({1}, fc2_input_scale,
            at::TensorOptions().device(routing_device).dtype(at::ScalarType::Float));
        ...
        workspace.adaptive_fc2_input_scale = adaptive_fc2_input_scale.data_ptr<float>();
        workspace.corrected_fc2_alpha = corrected_fc2_alpha.data_ptr<float>();
        workspace.adaptive_max_padded_tokens = static_cast<int32_t>(max_padded_tokens);
    }
