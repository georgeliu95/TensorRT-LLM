...
def _load_adaptive_fp4_ops() -> None:
    required_ops = ("fp4_quantize_ex", "calculate_global_amax")
    missing = [name for name in required_ops
               if not hasattr(torch.ops.trtllm, name)]
    if missing:
        raise RuntimeError(
            "Adaptive 4o6 operators are missing from this TensorRT-LLM build: "
            f"{', '.join(missing)}. Rebuild and install the current source tree."
        )
...
class NVFP4FusedMoEMethod(FusedMoEMethodBase):
    ...
    def _adaptive_4o6_has_vanilla_nvfp4_scales(
        ...
        return any(
            f"{expert_id}.{projection}.weight_scale" in weights
            for expert_id in expert_ids
    ...
    def _maybe_prepare_adaptive_4o6_weights_fused(
        ...
        # A dense checkpoint must still produce ordinary NVFP4 for a stage
        # whose adaptive flag is off. This keeps FC13-only and FC2-only modes
        # independent while giving the regular rc19 loader fully quantized
        # tensors for both projections.
        if dense_source or use_fc31:
            gate_up_weight = (adapted_weights["gate_up_proj"]
                              if dense_source else adapted_weights[
                                  "gate_up_proj"].clone())
            ...
            packed_gate_up = None
            packed_gate_up_scale = None
            fc31_rule = scale_rule if use_fc31 else standard_rule
            for expert_id in expert_ids:
                ...
                if dense_source:
                    if packed_gate_up is None:
                        packed_gate_up = torch.empty(
                            (gate_up_weight.shape[0], ) + fused_weight.shape,
                            dtype=fused_weight.dtype,
                            device=gate_up_weight.device)
                        packed_gate_up_scale = torch.empty(
                            (gate_up_weight.shape[0], ) + fused_scale.shape,
                            dtype=fused_scale.dtype,
                            device=gate_up_weight.device)
                    packed_gate_up[expert_id].copy_(
                        fused_weight.to(gate_up_weight.device))
                    packed_gate_up_scale[expert_id].copy_(
                        fused_scale.to(gate_up_weight.device))
                else:
                ...
                scale_2_overrides[(expert_id, "fc31")] = weight_scale_2
            if dense_source:
                if packed_gate_up is not None and packed_gate_up_scale is not None:
                    adapted_weights["gate_up_proj"] = packed_gate_up
                    adapted_weights["gate_up_proj_weight_scale"] = packed_gate_up_scale
        ...
        if dense_source or use_fc2:
            down_weight = (adapted_weights["down_proj"] if dense_source
                           else adapted_weights["down_proj"].clone())
            ...
            packed_down = None
            packed_down_scale = None
            fc2_rule = scale_rule if use_fc2 else standard_rule
            for expert_id in expert_ids:
                ...
                if dense_source:
                    ...
                    requantized, weight_scale_2 = (
                        _adaptive_4o6_requantize_nvfp4_group(
                            ...
                            scale_rule))
                fused_weight = requantized[0][0].transpose(0, 1).contiguous()
                fused_scale = requantized[0][1].transpose(0, 1).contiguous()
                if dense_source:
                    if packed_down is None:
                        packed_down = torch.empty(
                            (down_weight.shape[0], ) + fused_weight.shape,
                            dtype=fused_weight.dtype,
                            device=down_weight.device)
                        packed_down_scale = torch.empty(
                            (down_weight.shape[0], ) + fused_scale.shape,
                            dtype=fused_scale.dtype,
                            device=down_weight.device)
                    packed_down[expert_id].copy_(
                        fused_weight.to(down_weight.device))
                    packed_down_scale[expert_id].copy_(
                        fused_scale.to(down_weight.device))
                else:
                    down_weight[expert_id].copy_(
                        fused_weight.to(down_weight.device))
                    down_scale[expert_id].copy_(
                        fused_scale.to(down_scale.device))
                scale_2_overrides[(expert_id, "fc2")] = weight_scale_2
            if dense_source:
                if packed_down is not None and packed_down_scale is not None:
                    adapted_weights["down_proj"] = packed_down
                    adapted_weights["down_proj_weight_scale"] = packed_down_scale
